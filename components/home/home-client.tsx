...
export function HomeClient({
  ...
  const statsData = initialStats
  const mostPlayed = statsData?.leaderboard?.slice(0, 8) ?? []

  // Data is precomputed server-side once per day (see showOfTheDayService) and
  // passed down as a prop — no client-side fetch/loading state needed.
  const dayPayload = initialDayPayload
  ...
  const archiveCoveredIndices = useMemo((): Set<number> => {
                ...
                return (
                  <div key={set.name} className={`set-block${si === 1 ? ' alt' : ''}`}>
                    ...
                    {set.songs.map((song, ji) => {
                      ...
                      const inArchive = archiveCoveredIndices.has(globalNum)
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="play-dot" style={!inArchive ? { visibility: 'hidden' } : undefined}>▶</span>
                          <Link

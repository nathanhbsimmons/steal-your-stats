...
export function SongDetailClient({
  ...
  initialFacts: FirstLastFacts
  initialPositions: PositionFacts
  initialVersions: VersionsFacts
  initialTimeline: { performances: ShowRef[]; songTitle: string }
}) {
  ...
  const { currentTrack, isPlaying, prependToQueue, selectTrack } = usePlayer()

  const data = initialFacts
  const posData = initialPositions
  const versData = initialVersions
  const timelineData = initialTimeline

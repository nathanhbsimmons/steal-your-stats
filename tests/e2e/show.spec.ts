...
test.describe('Show detail page — setlist', () => {
  ...
  test('lists song titles in setlist', async ({ page }) => {
    await setupShowPage(page)
    await expect(page.getByRole('link', { name: 'Scarlet Begonias' })).toBeVisible({ timeout: 8_000 })
    await expect(page.getByRole('link', { name: 'Fire on the Mountain' })).toBeVisible({ timeout: 8_000 })
    await expect(page.getByRole('link', { name: 'Estimated Prophet' })).toBeVisible({ timeout: 8_000 })
    await expect(page.getByRole('link', { name: 'Morning Dew' })).toBeVisible({ timeout: 8_000 })
  })
  ...
  test('add-to-queue (+) buttons exist for each song', async ({ page }) => {
    await setupShowPage(page)
    await expect(page.locator('.add-q').first()).toBeVisible({ timeout: 8_000 })
    const count = await page.locator('.add-q').count()
